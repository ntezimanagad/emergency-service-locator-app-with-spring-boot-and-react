...
function Register() {
    ...
    const handleSendOtp = async (e) => {
        ...
        try {
            setLoading(true);
            setError("");
            const res = await axios.post("http://localhost:8080/api/auth/register/request-otp", { 
                username, 
                email, 
                password 
            });
    ...
    const handleVerifyOtp = async (e) => {
        ...
        try {
            setLoading(true);
            setError("");
            const res = await axios.post("http://localhost:8080/api/auth/register/validate-otp", { 
                email, 
                otpCode
            });
            
            if (res.data) {
                const userRes = await axios.post("http://localhost:8080/api/auth/register", { 
                    username, 
                    email, 
                    password 
                });
    ...
    const handleResendOtp = async () => {
        try {
            setLoading(true);
            setError("");
            const res = await axios.post("http://localhost:8080/api/auth/register/request-otp", { email });
            setTimer(60);
